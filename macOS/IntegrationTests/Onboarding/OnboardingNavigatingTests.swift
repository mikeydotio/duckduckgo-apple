...
final class OnboardingNavigatingTests: XCTestCase {
    ...
    func testOnImportData_DataImportViewShown() throws {
        ...
        let mvc = MainWindowController(
            window: mockWindow,
            mainViewController: MainViewController(tabCollectionViewModel: TabCollectionViewModel(isPopup: false), autofillPopoverPresenter: DefaultAutofillPopoverPresenter(pinningManager: MockPinningManager()), aiChatStateProvider: AIChatStateProvider(featureFlagger: MockFeatureFlagger()), fireCoordinator: fireCoordinator),
