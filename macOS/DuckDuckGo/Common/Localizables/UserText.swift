...
struct UserText {
    ...
    static let aiChatSubscriptionUpsellDialogSubscribeButton = NSLocalizedString("aichat.subscription-upsell-dialog.subscribe-button", value: "Subscribe to DuckDuckGo", comment: "Primary button in the subscription upsell dialog, shown when the user is still eligible for an introductory free trial (the trial is offered in the native purchase flow itself, not promised by this button's label)")
