...
final class ClickToLoadUserScript: NSObject, WKNavigationDelegate, Subfeature {
    ...
    private var shouldDisplayPlaceholdersWhenReady = false
    ...
    private func handleGetClickToLoadState(params: Any, message: UserScriptMessage) -> Encodable? {
        ...
        if shouldDisplayPlaceholdersWhenReady, let webView {
            shouldDisplayPlaceholdersWhenReady = false
            pushDisplayClickToLoadPlaceholders(into: webView)
        }
        return [
            "devMode": devMode,
    ...
    public func displayClickToLoadPlaceholders() {
        guard let webView else {
            shouldDisplayPlaceholdersWhenReady = true
            return
        }
    ...
    @MainActor
    private func pushDisplayClickToLoadPlaceholders(into webView: WKWebView) {
