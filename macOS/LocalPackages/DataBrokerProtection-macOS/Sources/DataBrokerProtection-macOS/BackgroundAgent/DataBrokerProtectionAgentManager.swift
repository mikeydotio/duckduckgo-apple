...
public final class DataBrokerProtectionAgentManager {
    ...
    // MARK: - MCP Debug Support (accessed by DataBrokerProtectionAgentManager+MCP.swift)

    lazy var browserWindowManager = BrowserWindowManager()
    var debugSessions: [String: DebugScanSession] = [:]
    let debugSessionsLock = NSLock()
    let debugSessionExpiry: TimeInterval = 30 * 60 // 30 minutes

    func createDebugSession() -> DebugScanSession {
        reapExpiredSessions()
    ...
    func debugSession(for sessionId: String?) -> DebugScanSession? {
        debugSessionsLock.lock()
    ...
    func removeDebugSession(_ session: DebugScanSession) async {
    ...
    func makeDebugEmailConfirmationService(for session: DebugScanSession) -> EmailConfirmationDataServiceProvider {
        let dbpSettings = DataBrokerProtectionSettings(defaults: .dbp)
    ...
    lazy var debugPixelHandler: EventMapping<DataBrokerProtectionSharedPixels> = EventMapping { event, _, _, _ in
        Logger.dataBrokerProtection.debug("Debug event: \(String(describing: event), privacy: .public)")
    }

    lazy var debugCaptchaService: CaptchaService = {
        let dbpSettings = DataBrokerProtectionSettings(defaults: .dbp)
