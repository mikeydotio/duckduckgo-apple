...
enum UserText {
    ...
    static let syncUpdateRequiredDescription = NSLocalizedString("alert.sync-update-required-description", bundle: Bundle.module, value: "This code is not supported in the current app version.", comment: "Description for Sync error shown when the app version does not support the scanned Sync code")
    static let syncUnsupportedThirdPartyRecoveryCodeTitle = NSLocalizedString("alert.sync-code-only-compatible-with-duckai-title", bundle: Bundle.module, value: "Scan this code from Duck.ai Settings.", comment: "Title for Sync error shown when a third-party recovery code can only be used with Duck.ai")
