...
public struct SwiftSuggestionProcessing {
    ...
    /// A `SuggestionProcessingHandler`-compatible entry point for the Swift implementation.
    public static func process(
        ...
        let processor = SwiftSuggestionProcessing(platform: platform)
        guard let result = processor.result(for: query, from: history, bookmarks: bookmarks,
                                            internalPages: internalPages, openTabs: openTabs,
                                            apiResult: apiResult) else {
            return .empty
        }
        return result
    }
    ...
    func result(for query: String,
        ...
        let duckDuckGoSuggestions = duckDuckGoSuggestions(from: apiResult) ?? []
        ...
        let allHistoryAndBookmarkAndOpenTabSuggestions = [
            bookmarks.compactMap(ScoringService.scored(lowercasedQuery: lowerQuery, queryTokens: queryTokens)),
            openTabs.compactMap(ScoringService.scored(lowercasedQuery: lowerQuery, queryTokens: queryTokens)),
            history.compactMap(ScoringService.scored(lowercasedQuery: lowerQuery, queryTokens: queryTokens)),
            internalPages.compactMap(ScoringService.scored(lowercasedQuery: lowerQuery, queryTokens: queryTokens)),
        ]
            .joined()
    ...
    /// Generates a list of phrase and website suggestions from the given API result.
        ...
        return result?.items
            .compactMap { suggestion in
                ...
                if suggestion.isNav == true {
                    guard let url = URL(string: URL.NavigationalScheme.http.separated() + phrase) else { return nil }
