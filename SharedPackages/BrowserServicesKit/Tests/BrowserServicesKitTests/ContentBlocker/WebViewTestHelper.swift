...
final class TestTrackerProtectionDelegate: NSObject, TrackerProtectionSubfeatureDelegate {
    ...
    /// Returns the first DetectedRequest matching `urlString` in either the
    /// blocked-trackers array or the third-party-requests array.
    func detectedRequest(for urlString: String) -> DetectedRequest? {
        detectedTrackers.first { $0.url == urlString }
            ?? detectedThirdPartyRequests.first { $0.url == urlString }
    }

    /// Returns true if a DetectedRequest was produced for `urlString`.
    func hasDetectedRequest(for urlString: String) -> Bool {
        detectedRequest(for: urlString) != nil
    }
    ...
    func trackerProtection(_ subfeature: TrackerProtectionSubfeature,
        ...
        if let detected = eventMapper.classifyResource(observation, adClickAttributionVendor: vendor) {
            ...
            } else if !eventMapper.isSameSiteObservation(observation) {
...
final class WebViewTestHarness: NSObject {
    ...
    ///   - privacyConfig: Privacy configuration for unprotected/exception list behavior.
    ///   - privacyConfigManager: Provides the JSON injected into C-S-S at runtime.
    ///   - temporaryUnprotectedDomains: Domains temporarily unprotected (EventMapper + content rules).
    ///   - userUnprotectedDomains: Domains user-unprotected (content rule exceptions + EventMapper).
    ///   - contentBlockingEnabled: Whether content blocking is logically enabled in the EventMapper.
    ///   - trackerExceptions: Tracker allowlist exceptions for WKContentRuleList compilation.
    ///   - proxy: A started `TestLoopbackProxy` instance.
    /// - Parameter useDefaultDataStore: When `true`, uses `WKWebsiteDataStore.default()` instead
    ///   of `.nonPersistent()`.  Required as a workaround for a macOS 26 WebKit crash triggered by
...
// MARK: - LEGACY -- MockRulesUserScriptDelegate
...
// MARK: - LEGACY -- MockSurrogatesUserScriptDelegate
...
// MARK: - LEGACY -- TestSchemeContentBlockerUserScriptConfig
...
// MARK: - LEGACY -- TestSchemeSurrogatesUserScriptConfig
...
// MARK: - LEGACY -- MockExperimentCohortsManager
