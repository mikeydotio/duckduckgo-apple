...
final class AccountManagerTests: XCTestCase {
    ...
    func testWhenCreatingAccountWithScopedAccessEnabledThenNativeCredentialSecretSeedIsPersisted() async throws {
        let api = RemoteAPIRequestCreatingMock()
        let endpoints = Endpoints(baseURL: Self.baseURL)
        var capturedPassword: String?
        var crypter = CryptingMock()
        crypter._createAccountCreationKeys = { _, password in
            capturedPassword = password
            return AccountCreationKeys(primaryKey: Data(repeating: 0x1, count: 32),
                                       secretKey: Data(repeating: 0x2, count: 32),
                                       protectedSecretKey: Data(repeating: 0x3, count: 32),
                                       passwordHash: Data(repeating: 0x4, count: 32))
        }
        ...
        let account = try await accountManager.createAccount(deviceName: "iPhone", deviceType: "iOS")

        let secret = try XCTUnwrap(capturedPassword)
        XCTAssertEqual(account.nativeCredentialSecret, secret)
        let decodedSecret = try XCTUnwrap(Base64URL.decode(secret))
        XCTAssertGreaterThanOrEqual(decodedSecret.count, 32)
        XCTAssertFalse(secret.contains("="))

        let body = try XCTUnwrap(api.createRequestCallArgs.last?.body)
        let payload = try XCTUnwrap(JSONSerialization.jsonObject(with: body) as? [String: Any])
        XCTAssertEqual(payload["credential_id"] as? String, "ddg")
    }

    func testWhenCreatingAccountWithScopedAccessDisabledThenNativeCredentialSecretStaysNil() async throws {
        let api = RemoteAPIRequestCreatingMock()
        let endpoints = Endpoints(baseURL: Self.baseURL)
        var capturedPassword: String?
        var crypter = CryptingMock()
        ...
        let account = try await accountManager.createAccount(deviceName: "iPhone", deviceType: "IPhone")

        XCTAssertNil(account.nativeCredentialSecret)
        XCTAssertNotNil(capturedPassword)

        let body = try XCTUnwrap(api.createRequestCallArgs.last?.body)
        let payload = try XCTUnwrap(JSONSerialization.jsonObject(with: body) as? [String: Any])
        XCTAssertNil(payload["credential_id"])
    }

    func testWhenDecodingLegacySyncAccountWithoutNativeCredentialSecretThenDecodingSucceeds() throws {
        let json = """
        {
            "device_id": "device-1",
            "device_name": "iPhone",
        ...
        let account = try JSONDecoder.snakeCaseKeys.decode(SyncAccount.self, from: Data(json.utf8))

        XCTAssertEqual(account.deviceId, "device-1")
        XCTAssertNil(account.nativeCredentialSecret)
    }
