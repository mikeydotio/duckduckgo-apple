...
public protocol SubJobWebRunning: CCFCommunicationDelegate {
    ...
    /// Email generated by a `generateEmail` action and forwarded to C-S-S as
    /// `data.fetchedEmail.email` on every subsequent userData payload. Consumed by `fillForm`
    /// actions that specify `dataSource: "fetchedEmail"`. Reset per job.
