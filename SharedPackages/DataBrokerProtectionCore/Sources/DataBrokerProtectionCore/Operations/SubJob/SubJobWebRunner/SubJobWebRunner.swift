...
public protocol SubJobWebRunning: CCFCommunicationDelegate {
    ...
    /// Cached email generated by a `generateEmailAction` earlier in the current job.
    /// When set, actions that would otherwise fetch a fresh email (e.g. `fillForm` with an email
    /// element) reuse this value instead. Intentionally one-way: `generateEmail` writes, other
    ...
    func runNextAction(_ action: Action) async {
        ...
        if let generateEmailAction = action as? GenerateEmailAction {
            guard fetchedEmail == nil else {
                ...
                resetRetriesCount()
                await onError(error: DataBrokerProtectionError.emailError(.alreadyGeneratedEmail))
                return
            }
            do {
                ...
                if let extractedProfile, let extractedProfileId = extractedProfile.id {
                    ...
                    emailData = try await emailConfirmationDataService.getEmail(
                        dataBrokerURL: context.dataBroker.url,
                        attemptId: stageCalculator.attemptId
                    )
                }
