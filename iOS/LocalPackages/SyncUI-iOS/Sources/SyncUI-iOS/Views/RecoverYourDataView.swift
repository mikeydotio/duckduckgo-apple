...
struct RecoverYourDataView: View {
    ...
    private var doneButton: some View {
        if #available(iOS 26.0, *) {
            Button(action: model.recoverYourDataDoneFromConnectingSheet) {
    ...
    private var headerSection: some View {
        Section {
            VStack(spacing: 8) {
                Image("QR-Download-Feature-128", bundle: .module)
                    .resizable()
                    .aspectRatio(contentMode: .fit)
                    .frame(width: 128, height: 96)

                Text(UserText.simplifiedRecoverYourDataV2Title)
                    .daxTitle1()
                    .multilineTextAlignment(.center)
                    .foregroundColor(Color(designSystemColor: .textPrimary))

                Text(UserText.simplifiedRecoverYourDataV2Description)
                    .daxBodyRegular()
...
private extension SyncSettingsViewModel {
    static func premodel(isAutoRestoreAvailable: Bool = true) -> SyncSettingsViewModel {
...
#Preview("Recover Your Data") {
    RebrandedPreview(isRebranded: true) {
        RecoverYourDataView(model: .premodel())
    }
}

#Preview("Recover Your Data – No Auto-Restore") {
    RebrandedPreview(isRebranded: true) {
        RecoverYourDataView(model: .premodel(isAutoRestoreAvailable: false))
    }
}
